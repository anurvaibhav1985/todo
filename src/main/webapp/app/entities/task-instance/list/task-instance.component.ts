...
export class TaskInstanceComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.taskInstanceService
      ...
      .subscribe(
        (res: HttpResponse<ITaskInstance[]>) => {
  ...
  trackId(index: number, item: ITaskInstance): number {
    return item.id!;
  }

  delete(taskInstance: ITaskInstance): void {
    const modalRef = this.modalService.open(TaskInstanceDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: ITaskInstance[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/task-instance'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
