...
/**
 * Performance test for the TaskInstance entity.
 */
class TaskInstanceGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the TaskInstance entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all taskInstances")
            .get("/api/task-instances")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new taskInstance")
            ...
            .body(StringBody("""{
                ...
                , "plannedEndDate":"2020-01-01T00:00:00.000Z"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_taskInstance_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created taskInstance")
                .get("${new_taskInstance_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created taskInstance")
            .delete("${new_taskInstance_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
